...
export const SectionHeader: React.FC<{
  label: string; title: string; subtitle?: string; center?: boolean; dark?: boolean;
  ...
  <div style={{ textAlign: center ? 'center' : 'left', marginBottom: '3.5rem' }}>
    ...
    <div style={{
      fontFamily: "'Syne',sans-serif", fontSize: 'clamp(1.8rem, 3.2vw, 2.4rem)',
      fontWeight: 700, color: dark ? '#fff' : '#121420',
      lineHeight: 1.15, marginBottom: 14, letterSpacing: '-0.01em',
      whiteSpace: 'pre-line',
    }}>{title}</div>
    {subtitle && (
      <div style={{
        fontSize: '1.05rem', color: dark ? 'rgba(255,255,255,0.7)' : '#6B7089',
        lineHeight: 1.7, maxWidth: 600, margin: center ? '0 auto' : '0',
      }}>{subtitle}</div>
    )}
  </div>
);
...
export const ServiceCard: React.FC<{
  icon: string; title: string; desc: string; accent: 'blue' | 'green'; image: string; tag?: string;
  ...
  return (
    <div
      ...
      style={{
        background: '#fff', border: '1px solid #E4E6EF', borderRadius: 20,
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          transform: hovered ? 'translateY(-4px) rotate(-6deg)' : 'translateY(0) rotate(0)',
          transition: 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)',
          zIndex: 2,
        }}>
          <svg viewBox="0 0 24 24" width="24" height="24" fill="none"
            stroke={accentDark} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
            <path d={icon} />
          </svg>
        </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          fontWeight: 700, color: '#121420', marginBottom: 10,
          letterSpacing: '-0.01em',
        }}>{title}</div>
        <div style={{
          fontSize: '0.9rem', color: '#6B7089',
...
export const JourneyTimeline: React.FC<{
  milestones: Milestone[]; lang: Lang; label: string; title: string; subtitle?: string;
    ...
    <div style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
        <span style={{
          ...
          fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em',
          background: '#D6E4F7', color: '#1B4FBB', marginBottom: 16,
        }}>
        ...
        <div style={{
          ...
          marginBottom: 14, letterSpacing: '-0.01em', whiteSpace: 'pre-line',
        }}>{title}</div>
        {subtitle && (
          <div style={{
            fontSize: '1.05rem', color: '#6B7089',
      ...
      {/* Timeline */}
      <div style={{ position: 'relative', maxWidth: 1040, margin: '0 auto', padding: '0 0 24px' }}>
        {/* Centerline */}
        <div data-timeline-centerline style={{
          position: 'absolute', left: '50%', top: 0, bottom: 0,
          width: 2, transform: 'translateX(-50%)',
          background: 'linear-gradient(180deg, #D6E4F7 0%, #1B4FBB 30%, #7DB83A 100%)',
          borderRadius: 2,
        }} />
        ...
        {milestones.map((m, i) => {
          ...
          return (
            <div key={i} data-timeline-row style={{
              position: 'relative',
              display: 'grid', gridTemplateColumns: '1fr 88px 1fr',
              gap: 0, marginBottom: i < milestones.length - 1 ? 56 : 0,
              alignItems: 'center',
            }}>
              {/* Left card slot */}
              <div style={{ gridColumn: 1, paddingInlineEnd: 24 }}>
                {!cardOnRight && (
                  <MilestoneCard m={m} isActive={isActive} side="left" accentColor={accentColor} accentDark={accentDark} accentBg={accentBg} onEnter={() => setActiveIdx(i)} onLeave={() => setActiveIdx(null)} isRTL={isRTL} />
                )}
              </div>

              {/* Center node */}
              <div style={{ gridColumn: 2, display: 'flex', justifyContent: 'center', position: 'relative', zIndex: 2 }}>
                <div style={{
                  ...
                  {m.ongoing ? (
                    <span style={{
                      ...
                      animation: 'milestoneOngoingDot 1.6s ease-in-out infinite',
                    }} />
                  ) : m.year}
                  {/* Pulse ring */}
                  <div style={{
              ...
              <div style={{ gridColumn: 3, paddingInlineStart: 24 }}>
                {cardOnRight && (
                  <MilestoneCard m={m} isActive={isActive} side="right" accentColor={accentColor} accentDark={accentDark} accentBg={accentBg} onEnter={() => setActiveIdx(i)} onLeave={() => setActiveIdx(null)} isRTL={isRTL} />
                )}
              </div>
            </div>
          );
...
const MilestoneCard: React.FC<{
  m: Milestone; isActive: boolean; side: 'left' | 'right';
  accentColor: string; accentDark: string; accentBg: string;
  onEnter: () => void; onLeave: () => void; isRTL: boolean;
}> = ({ m, isActive, side, accentColor, accentDark, accentBg, onEnter, onLeave, isRTL }) => {
  // arrow points toward the centerline
  const arrowSide = side === 'left' ? (isRTL ? 'left' : 'right') : (isRTL ? 'right' : 'left');
  return (
    <div
      data-timeline-card
      ...
      style={{
        ...
        transition: 'all 0.35s cubic-bezier(0.34,1.4,0.5,1)',
        textAlign: side === 'left' ? (isRTL ? 'left' : 'right') : (isRTL ? 'right' : 'left'),
      }}
    >
      {/* Pointer arrow toward centerline */}
      <div data-timeline-arrow style={{
        position: 'absolute',
        top: 22,
        [arrowSide]: -8,
        width: 16, height: 16,
        ...
        transform: arrowSide === 'right' ? 'rotate(-45deg)' : 'rotate(135deg)',
      }} />
      ...
      {m.tag && (
        <span style={{
          ...
          background: accentBg, color: accentDark, marginBottom: 10,
        }}>{m.tag}</span>
      )}
      ...
      {/* Description */}
      <div style={{
        fontSize: 14, color: '#6B7089', lineHeight: 1.65,
      ...
      {m.stat && (
        <div style={{
          ...
          <span style={{
            ...
            color: accentDark, letterSpacing: '-0.02em', lineHeight: 1,
            fontVariantNumeric: 'tabular-nums', fontFeatureSettings: '"tnum"',
          }}>{m.stat.n}</span>
          <span style={{
            fontSize: 12, color: '#6B7089', lineHeight: 1.3,
            fontFamily: "'DM Sans',sans-serif",
          }}>{m.stat.l}</span>
        </div>
      )}
...
export const ProgramCard: React.FC<{
  tag: string; title: string; desc: string; duration?: string; hours?: string; accent: 'blue' | 'green';
}> = ({ tag, title, desc, duration, hours, accent }) => {
  const [hovered, setHovered] = React.useState(false);
  return (
    <div
      onMouseEnter={() => setHovered(true)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        ...
        {hours && (
          <span style={{
            ...
            color: '#0D2D72', letterSpacing: '-0.02em',
          }}>{hours}</span>
        )}
      </div>
      <div style={{
        fontFamily: "'Syne',sans-serif", fontSize: '1.25rem',
        fontWeight: 700, color: '#121420', lineHeight: 1.25,
      }}>{title}</div>
      <div style={{ fontSize: '0.95rem', color: '#6B7089', lineHeight: 1.65, flex: 1 }}>{desc}</div>
      {duration && (
        <div style={{
          fontSize: 12, fontWeight: 600, color: '#1B4FBB',
          paddingTop: 12, borderTop: '1px solid #E4E6EF',
...
export const StartupCard: React.FC<{
  tag: string; name: string; desc: string; faculty?: string; year?: string;
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
        <span style={{
          ...
          fontSize: 11, fontWeight: 700, background: '#EBF5D8', color: '#5A8A22',
          letterSpacing: '0.03em',
        }}>{tag}</span>
        {year && (
          <span style={{ fontSize: 11, color: '#6B7089', fontFamily: "'DM Sans',sans-serif" }}>{year}</span>
        )}
      </div>
      <div style={{
        fontFamily: "'Syne',sans-serif", fontSize: '1.15rem',
        fontWeight: 700, color: '#121420', marginBottom: 8, letterSpacing: '-0.01em',
      }}>{name}</div>
      <div style={{ fontSize: '0.875rem', color: '#6B7089', lineHeight: 1.6, flex: 1, marginBottom: 14 }}>{desc}</div>
      {faculty && (
        <div style={{
          fontSize: 11, fontWeight: 600, color: '#6B7089',
...
export const TeamCard: React.FC<{
  name: string; photo?: string | null; accent?: 'blue' | 'green';
  ...
  const accentGradient = accent === 'green'
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: 'transform 0.35s cubic-bezier(0.34,1.4,0.5,1), box-shadow 0.35s ease, border-color 0.25s ease',
    ...
    >
      ...
      <div style={{
        ...
        <div aria-hidden="true" style={{
          ...
          background: 'linear-gradient(to top, rgba(13,45,114,0.55) 0%, rgba(13,45,114,0) 100%)',
      ...
      <div style={{
        ...
        <div style={{
          ...
          color: '#121420', lineHeight: 1.25,
          letterSpacing: '-0.005em',
        }}>{name}</div>
        {/* Tiny accent bar — replaces former role/faculty meta */}
        <div style={{
          width: hovered ? 36 : 24, height: 2, borderRadius: 2,
...
/* ─── FOOTER ─── */
export const Footer: React.FC<{ onNav: (id: string) => void; lang: Lang }> = ({ onNav, lang }) => {
  const isRTL = lang === 'AR';

  const cols: { title: Record<Lang, string>; items: (Record<Lang, string> & { key?: string })[] }[] = [
    {
      title: { EN: "Business Incubator M'Sila", FR: "Business Incubator M'Sila", AR: "حاضنة أعمال مسيلة" },
      items: [
    ...
    },
    {
      ...
      items: [
        ...
        { EN: 'Pre-incubation', FR: 'Pré-incubation', AR: 'ما قبل الحضانة', key: 'programs' },
        { EN: 'Training Bootcamp', FR: 'Bootcamp', AR: 'مخيم التدريب', key: 'programs' },
      ],
    },
    {
      title: { EN: 'Services', FR: 'Services', AR: 'الخدمات' },
      items: [
        { EN: 'Legal Support', FR: 'Support juridique', AR: 'الدعم القانوني', key: 'services' },
        { EN: 'Funding Access', FR: 'Accès au financement', AR: 'الوصول للتمويل', key: 'services' },
        { EN: 'Co-working Space', FR: 'Espace co-working', AR: 'فضاء العمل', key: 'services' },
      ],
    },
    {
      title: { EN: 'Contact', FR: 'Contact', AR: 'تواصل' },
      items: [
        { EN: 'Apply for Incubation', FR: 'Candidater', AR: 'تقدّم للحضانة', key: 'contact' },
        { EN: 'Send a Message', FR: 'Nous écrire', AR: 'أرسل رسالة', key: 'contact' },
      ],
    },
  ];
  ...
  return (
    <footer style={{
      background: '#0A0E1F', color: '#fff',
      padding: '4.5rem 2rem 2rem',
      direction: isRTL ? 'rtl' : 'ltr',
      borderTop: '1px solid rgba(255,255,255,0.05)',
    }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        <div data-footer-grid style={{ display: 'grid', gridTemplateColumns: '1.5fr repeat(4, 1fr)', gap: '2.5rem', marginBottom: '3.5rem' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <img src={logo} alt="" style={{ height: 44, width: 44, borderRadius: '50%' }} />
              <div style={{ lineHeight: 1.1 }}>
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800, color: '#fff', letterSpacing: '-0.005em' }}>Business Incubator</div>
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 14, fontWeight: 700, color: '#7DB83A', marginTop: 2 }}>M'Sila</div>
              </div>
            </div>
            <p style={{
            ...
            <div key={ci}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {col.items.map((item, ii) => (
                  <button key={ii} onClick={() => item.key && onNav(item.key)} style={{
                  ...
                  onMouseEnter={e => { if (item.key) e.currentTarget.style.color='#fff'; }}
